...
function mdInline(text, footnotes) {
  ...
  text = text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, function(_, alt, src) {
    var i = _imgs.length;
    var isUnsplash = src.includes('unsplash.com');
    var srcset = '';
    if (isUnsplash) {
    ...
    var img = '<img src="' + src + '" alt="' + alt + '" loading="lazy" decoding="async" fetchpriority="low"' + srcset + '>';
  ...
  if (footnotes) {
    text = text.replace(/\[\^([^\]]+)\]/g, function(_, name) {
      if (footnotes[name] !== undefined) {
        return '<sup class="footnote-ref"><a href="#fn-' + name + '" id="fnref-' + name + '" data-fn-name="' + name + '">' + footnotes[name] + '</a></sup>';
      }
...
function mdToHtml(src) {
  ...
  while (i < lines.length) {
    ...
    if (line.match(/^```/)) {
      if (inCodeBlock) {
        ...
        const lang = langParts[0] || 'code';
        const codeFile = langParts.slice(1).join(' ');
        if (lang === 'mermaid') {
          html += '<div class="mermaid-block" data-code="' + code.replace(/&/g, '&amp;').replace(/"/g, '&quot;') + '"><div class="mermaid-loading">Loading diagram...</div></div>\n';
        } else {
          const fileAttr = codeFile ? ' file-title="' + escHtml(codeFile) + '"' : '';
          html += '<pre><code class="language-' + lang + '"' + fileAttr + '>' + escHtml(code) + '</code></pre>\n';
        }
        inCodeBlock = false;
        codeLines = [];
        codeLang = '';
    ...
    if (line.includes('|') && (line.trim().startsWith('|') || line.trim().match(/^[^|]*\|/))) {
      ...
      // Separator row detection: ALL non-empty cells must be separator pattern (---, :---, ---:, :---:)
      const nonEmpty = cells.filter(c => c.trim() !== '');
      const isSeparator = nonEmpty.length > 0 && nonEmpty.every(c => /^[:\s]*-{3,}[\s:]*$/.test(c.trim()));
      if (isSeparator) {
  ...
  if (fnDefs.length) {
    ...
    fnDefs.forEach(fn => {
      html += '<li id="fn-' + fn.name + '">' + mdInline(fn.text, footnotes) + ' <a href="#fnref-' + fn.name + '" class="footnote-backref" data-fn-name="' + fn.name + '">↩</a></li>\n';
    });
    html += '</ol></section>\n';
  }
