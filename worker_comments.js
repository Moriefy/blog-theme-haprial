...
async function sendReplyNotification(env, parentEmail, parentContent, replyNickname, replyHtml, pageSlug, replyEmail) {
  if (!env.RESEND_API_KEY || !parentEmail) return;
  // 不给自己发通知
  if (replyEmail && parentEmail === replyEmail) return;

  const siteUrl = env.SITE_URL || 'https://pluslogic.eu.org';
  const fromAddr = env.NOTIFY_FROM || 'Haprial Blog <noreply@pluslogic.eu.org>';
  const articleUrl = siteUrl + pageSlug;

  // 被回复的原评论内容预览（纯文本，最多 200 字）
  const preview = (parentContent || '').replace(/<[^>]+>/g, '').slice(0, 200);

  const subject = `💬 ${replyNickname} 回复了你的评论`;
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1a1a1a;max-width:560px;margin:0 auto;padding:24px;">
<div style="margin-bottom:20px;"><a href="${siteUrl}" style="font-size:18px;font-weight:600;color:#1a1a1a;text-decoration:none;">Moriefyの半岛铁盒</a></div>
<p style="font-size:14px;color:#555;margin:0 0 16px;">${esc(replyNickname)} 回复了你在 <a href="${articleUrl}" style="color:#2563eb;text-decoration:none;">这篇文章</a> 下的评论：</p>
<div style="background:#f5f5f5;border-left:3px solid #2563eb;padding:12px 16px;border-radius:4px;margin:0 0 16px;font-size:14px;color:#333;line-height:1.6;">${preview || '(无内容)'}</div>
<div style="background:#f9f9f9;border:1px solid #e5e5e5;padding:12px 16px;border-radius:4px;margin:0 0 20px;font-size:14px;color:#333;line-height:1.6;">${replyHtml}</div>
<a href="${articleUrl}" style="display:inline-block;background:#1a1a1a;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;font-size:14px;font-weight:500;">查看回复</a>
<hr style="border:none;border-top:1px solid #e5e5e5;margin:24px 0;">
<p style="font-size:12px;color:#999;">此邮件由博客评论系统自动发送。如果不想收到通知，下次评论时可以不填写邮箱。</p>
</body></html>`;
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: fromAddr, to: [parentEmail], subject, html })
    });
  } catch (e) {
    // 静默失败，不影响评论功能
  }
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (method === 'POST' && path === '/api/comments') {
        ...
        // 异步发送回复通知（不阻塞响应）
        if (parentId > 0) {
          const parentComment = await env.DB.prepare("SELECT nickname,email,content FROM comments WHERE id=? AND status='approved'").bind(parentId).first();
          if (parentComment && parentComment.email) {
            ctx.waitUntil(sendReplyNotification(env, parentComment.email, parentComment.content, nickname, contentHtml, page, email).catch(() => {}));
          }
        }
      ...
      if (path.startsWith('/api/admin/')) {
        ...
        if (method === 'POST' && path === '/api/admin/comments') {
          ...
          const contentHtml = renderMd(content);
          const _parentId = parent_id || 0;
          const result = await env.DB.prepare("INSERT INTO comments (page_slug,parent_id,depth,nickname,email,website,avatar_hash,content,content_html,ip,user_agent,status,is_admin) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(page, _parentId, depth || 0, nickname || 'Moriefy', email || '3518972914@qq.com', website || 'https://pluslogic.eu.org', avatarHash, content, contentHtml, 'admin', 'Admin Dashboard', 'approved', 1).run();

          // 管理员回复也发通知
          if (_parentId > 0) {
            const parentComment = await env.DB.prepare("SELECT nickname,email,content FROM comments WHERE id=? AND status='approved'").bind(_parentId).first();
            if (parentComment && parentComment.email) {
              ctx.waitUntil(sendReplyNotification(env, parentComment.email, parentComment.content, nickname || 'Moriefy', contentHtml, page, email || '3518972914@qq.com'));
            }
          }
