...
function loadArticle(id){var art=articles[id];if(!art)return;loadArticleMeta(id);rprog.style.transform='scaleX(0)';fab.classList.remove('visible');articleView.scrollTop=0;headingCache=null;lastActiveId=null;tocManualLock=false;clearTimeout(tocManualTimer);clearTimeout(tocBuildTimer);_lastArtScrolled=null;_lastArtFab=null;articleBody.classList.remove('revealed');function renderContent(html){if(currentArticleId!==id)return;articleBody.innerHTML=html;_artScrollMax=articleView.scrollHeight-articleView.clientHeight;articleBody.classList.add('revealed');tocBuildTimer=setTimeout(function(){if(currentArticleId===id)buildTOC()},80);var _cm=window.__HAPRIAL_DATA__.comments;if(_cm&&_cm.enabled&&_cm.api){__initComments('/posts/'+id+'/')}setTimeout(function(){if(currentArticleId!==id)return;codeProcessed=false;processCodeBlocks();revealImages();observeImages();processMermaid()},50);setTimeout(function(){if(currentArticleId===id)prefetchNeighbors(id)},300);var _sv=articleView.scrollTop;fab.classList.toggle('visible',_sv>400);fabComment.classList.toggle('shifted',_sv>400);_lastArtFab=(_sv>400)}if(_contentCache[id]){renderContent(_contentCache[id]);var ci=_cacheOrder.indexOf(id);if(ci>0){_cacheOrder.splice(ci,1);_cacheOrder.push(id)}return}articleBody.innerHTML='<div class="skel-wrap"><div class="skel-line skel-w60"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w90"></div><div class="skel-line skel-w70"></div><div class="skel-gap"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w80"></div><div class="skel-line skel-w95"></div><div class="skel-line skel-w60"></div><div class="skel-gap"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w75"></div><div class="skel-line skel-w85"></div></div>';fetch('/posts/'+id+'/article.json').then(function(r){if(!r.ok)throw new Error(r.status);return r.json()}).then(function(d){cachePut(id,d.content||'');renderContent(_contentCache[id])}).catch(function(){articleBody.innerHTML='<p style="text-align:center;padding:80px 0;color:var(--outline)">加载失败，请刷新重试。</p>'})}
function openArticleVisual(id){scrollPos=window.scrollY;articleView.classList.add('open');articleBody.innerHTML='';requestAnimationFrame(function(){lockBody();loadArticle(id);avBar.classList.remove('scrolled');fabComment.classList.add('visible');fabComment.classList.remove('shifted');var af=document.getElementById('anti-fouc');if(af)af.remove()})}
...
function closeArticleVisual(){if(!articleView.classList.contains('open'))return;if(currentArticleId)saveArticleScroll(currentArticleId);currentArticleId=null;clearTimeout(tocBuildTimer);if(imgObserver){imgObserver.disconnect();imgObserver=null}articleBody.querySelectorAll('.mermaid-block').forEach(function(b){if(b._zoomCleanup){b._zoomCleanup();delete b._zoomCleanup}});closeTocSheet();window.scrollTo(0,scrollPos);articleView.classList.remove('open');unlockBody();requestAnimationFrame(function(){var af=document.getElementById('anti-fouc');if(af)af.remove();_lastFab=null;fabComment.classList.remove('visible');fabComment.classList.remove('shifted');fab.classList.remove('idle');fabComment.classList.remove('idle');clearTimeout(_fabIdleTimer);lastRouteWasArticle=false;topAppBar.classList.toggle('elevated',scrollPos>0);fab.classList.toggle('visible',scrollPos>400)})}
