...
var headingCache=null,lastActiveId=null,scrollRaf=null,artScrollRaf=null,resizeTimer=null;
var paginationPage=0,savedPaginationPage=0,perPage=6;
var currentArticleId=null,isSwitchingArticle=false;
var searchTimer=null,allCardsCache=null;
...
var siteWrapper=$('siteWrapper'),topAppBar=$('topAppBar'),articleView=$('articleView'),articleBody=$('articleBody'),
...
function getAllCards(){if(!allCardsCache)allCardsCache=[].slice.call(document.querySelectorAll('.card'));return allCardsCache}
function invalidateCards(){allCardsCache=null}
var prismLoaded=false;
function revealImages(){var imgs=articleBody.querySelectorAll('img');for(var i=0;i<imgs.length;i++){var img=imgs[i];if(img.classList.contains('revealed'))continue;if(reducedMotion){img.style.opacity='1';img.style.transform='';img.classList.add('revealed');continue}if(img.complete&&img.naturalWidth>0){img.classList.add('revealed');continue}(function(im){im.addEventListener('load',function(){requestAnimationFrame(function(){im.classList.add('revealed')})},{once:true});im.addEventListener('error',function(){im.style.opacity='1';im.style.transform='';im.classList.add('revealed')},{once:true})})(img)}}
var imgObserver=null;
function observeImages(){if(imgObserver)imgObserver.disconnect();if(reducedMotion){var imgs=articleBody.querySelectorAll('img');for(var i=0;i<imgs.length;i++)imgs[i].classList.add('revealed');return}imgObserver=new IntersectionObserver(function(entries){for(var i=0;i<entries.length;i++){if(entries[i].isIntersecting){var img=entries[i].target;imgObserver.unobserve(img);if(img.complete&&img.naturalWidth>0){requestAnimationFrame((function(im){return function(){im.classList.add('revealed')}})(img))}}}},{root:articleView,rootMargin:'100px',threshold:0.05});var imgs=articleBody.querySelectorAll('img:not(.revealed)');for(var i=0;i<imgs.length;i++)imgObserver.observe(imgs[i])}

function processCodeBlocks(){if(codeProcessed)return;codeProcessed=true;var pres=articleBody.querySelectorAll('pre');if(!pres.length)return;if(!document.querySelector('link[href*="prism-tomorrow"]')){var cb=document.createElement('link');cb.rel='stylesheet';cb.href='https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/themes/prism-tomorrow.min.css';document.head.appendChild(cb)}function highlightAll(pres){var frag=document.createDocumentFragment();var replacements=[];for(var i=0;i<pres.length;i++){var pre=pres[i];if(pre.closest('.code-block'))continue;var code=pre.querySelector('code'),text=code?code.textContent:pre.textContent;text=text.replace(/^\n+/,'').replace(/\n+$/,'');var lines=text.split('\n');var lm=code?code.className.match(/language-(\w+)/):null,lang=lm?lm[1]:'code';var w=document.createElement('div');w.className='code-block';w.innerHTML='<div class="cb-bar"><span class="cb-lang">'+lang+'</span><button class="cb-copy" type="button" aria-label="复制代码">'+icons.copy+'</button></div><div class="cb-body"><div class="cb-lines">'+lines.map(function(_,j){return'<span>'+(j+1)+'</span>'}).join('')+'</div><div class="cb-sep"></div><div class="cb-pre-wrap"><pre class="language-'+lang+'"><code class="language-'+lang+'"></code></pre></div></div>';w.querySelector('code').textContent=lines.join('\n');replacements.push({old:pre,new:w})}for(var i=0;i<replacements.length;i++){replacements[i].old.parentNode.replaceChild(replacements[i].new,replacements[i].old)}if(typeof Prism!=='undefined'&&window.Prism){try{Prism.highlightAll()}catch(e){}}}if(prismLoaded){highlightAll(pres);return}prismLoaded=true;var s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/prism.min.js';s.setAttribute('data-manual','');s.onload=function(){if(typeof Prism!=='undefined')highlightAll(pres)};document.head.appendChild(s)}
function applyStagger(){/* no-op: stagger removed for performance */}
function setPageStagger(pid){var el=document.getElementById(pid);if(!el)return;var items=el.querySelectorAll('.tag-card,.cat-card,.fl-card');for(var i=0;i<items.length;i++)items[i].style.setProperty('--sd',(i*40)+'ms')}
...
function renderPaginationPage(noStagger){var all=getAllCards(),cards=[];for(var i=0;i<all.length;i++){if(all[i].dataset._filtered!=='1')cards.push(all[i])}var total=Math.ceil(cards.length/perPage);if(total<1)total=1;if(paginationPage>=total)paginationPage=total-1;if(paginationPage<0)paginationPage=0;try{sessionStorage.setItem('pg',paginationPage)}catch(e){}var start=paginationPage*perPage,end=start+perPage;for(var i=0;i<all.length;i++){var c=all[i];if(c.dataset._filtered==='1'){c.style.display='none';c.classList.remove('show')}}var shown=[];for(var i=0;i<cards.length;i++){var c=cards[i];if(i>=start&&i<end){c.style.display='';c.classList.remove('show');shown.push(c)}else{c.style.display='none';c.classList.remove('show')}}if(shown.length){if(noStagger||isLowEnd){for(var i=0;i<shown.length;i++)shown[i].classList.add('show')}else{for(var i=0;i<shown.length;i++){shown[i].style.setProperty('--ci',i);shown[i].classList.add('show')}}}var noR=$('noResults');if(!cards.length){noR.classList.add('show');pagination.innerHTML='';pagination.classList.remove('show');return}noR.classList.remove('show');if(total>1){var parts=['<button class="pg-btn"'+(paginationPage===0?' disabled':'')+' data-pg="prev" aria-label="上页">'+icons.chevL+'</button>'];for(var i=0;i<total;i++)parts.push('<button class="pg-btn'+(i===paginationPage?' active':'')+'" data-pg="'+i+'" aria-label="第'+(i+1)+'页">'+(i+1)+'</button>');parts.push('<button class="pg-btn"'+(paginationPage===total-1?' disabled':'')+' data-pg="next" aria-label="下页">'+icons.chevR+'</button>');pagination.innerHTML=parts.join('');pagination.classList.add('show')}else{pagination.innerHTML='<span style="font-size:13px;color:var(--outline)">共 '+cards.length+' 篇文章</span>';pagination.classList.add('show')}}
function renderAllCards(){var all=getAllCards();for(var i=0;i<all.length;i++){all[i].dataset._filtered='';all[i].style.display='';all[i].classList.remove('show')}}

function setTocHighlight(id){lastActiveId=id;var activeItem=null;var items=tocList.children;for(var i=0;i<items.length;i++){var it=items[i];var isActive=it.dataset.target===id;it.classList.toggle('active',isActive);if(isActive)activeItem=it}var dItems=tocDrawerList.children;for(var i=0;i<dItems.length;i++){dItems[i].classList.toggle('active',dItems[i].dataset.target===id)}if(activeItem){var aTop=activeItem.offsetTop;var sTop=toc.scrollTop;var cH=toc.clientHeight;if(aTop<sTop||aTop+activeItem.offsetHeight>sTop+cH){toc.scrollTop=aTop-cH/2}}}
...
function buildTOC(){var fragToc=document.createDocumentFragment();var fragDrawer=document.createDocumentFragment();headingCache=null;lastActiveId=null;var heads=articleBody.querySelectorAll('h2,h3');for(var i=0;i<heads.length;i++){var h=heads[i];var li=document.createElement('li');li.className='toc-item';li.dataset.target=h.id;li.innerHTML='<a class="toc-link'+(h.tagName==='H3'?' sub':'')+'">'+h.textContent+'</a>';fragToc.appendChild(li);li.addEventListener('click',(function(hid,htop){return function(){lockTocManual();setTocHighlight(hid);articleView.scrollTo({top:htop-80,behavior:'smooth'})}})(h.id,h.offsetTop));var di=document.createElement('li');di.className='toc-drawer-item'+(h.tagName==='H3'?' sub':'');di.textContent=h.textContent;di.dataset.target=h.id;fragDrawer.appendChild(di);di.addEventListener('click',(function(hid,htop){return function(){closeTocSheet();lockTocManual();setTocHighlight(hid);articleView.scrollTo({top:htop-80,behavior:'smooth'})}})(h.id,h.offsetTop))}tocList.innerHTML='';tocDrawerList.innerHTML='';tocList.appendChild(fragToc);tocDrawerList.appendChild(fragDrawer);cacheHeadings();if(headingCache.length)setTocHighlight(headingCache[0].id);setTimeout(function(){toc.classList.add('visible')},200)}
function cacheHeadings(){headingCache=[];articleBody.querySelectorAll('h2,h3').forEach(function(h){headingCache.push({id:h.id,top:h.offsetTop})})}
...
function openTocSheet(){tocSheet.classList.add('open');setTimeout(function(){document.addEventListener('click',tocOutClick)},10)}
...
function loadArticle(id){var art=articles[id];if(!art)return;loadArticleMeta(id);rprog.style.transform='scaleX(0)';fab.classList.remove('visible');articleBody.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--outline)">加载中…</div>';articleView.scrollTop=0;headingCache=null;lastActiveId=null;tocManualLock=false;clearTimeout(tocManualTimer);_lastArtScrolled=null;_lastArtFab=null;function onContent(html){articleBody.innerHTML=html;codeProcessed=false;processCodeBlocks();applyStagger();revealImages();observeImages();articleBody.classList.remove('revealed');if(!isLowEnd){requestAnimationFrame(function(){articleBody.classList.add('revealed')})}else{articleBody.classList.add('revealed')}setTimeout(buildTOC,80);__initTwikoo('/posts/'+id+'/')}if(__articleCache[id]){onContent(__articleCache[id]);return}fetch('/posts/'+id+'/article.json').then(function(r){return r.json()}).then(function(d){__articleCache[id]=d.content;onContent(d.content)}).catch(function(){articleBody.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--outline)">加载失败，请刷新重试</div>'})}
function openArticleVisual(id){scrollPos=siteWrapper.scrollTop;loadArticle(id);lockBody();articleView.classList.add('open');avBar.classList.remove('scrolled');fabComment.classList.add('visible');fabComment.classList.remove('shifted')}
function openArticleVisualImmediate(id){loadArticle(id);lockBody();articleView.style.transition='none';articleView.classList.add('open');articleView.offsetHeight;articleView.style.transition='';avBar.classList.remove('scrolled');fabComment.classList.add('visible');fabComment.classList.remove('shifted')}
function switchToArticleVisual(nid,dir){if(isSwitchingArticle)return;isSwitchingArticle=true;var ac=$('avContent'),dx=dir==='next'?-16:16;ac.style.transition='opacity 160ms cubic-bezier(.4,0,1,1),transform 160ms cubic-bezier(.4,0,1,1)';ac.style.opacity='0';ac.style.transform='translateX('+dx+'px)';function onFadeOut(){ac.removeEventListener('transitionend',onFadeOut);ac.style.transition='none';ac.style.transform='translateX('+(-dx)+'px)';loadArticle(nid);void ac.offsetHeight;requestAnimationFrame(function(){ac.style.transition='opacity 240ms cubic-bezier(0,0,.2,1),transform 240ms cubic-bezier(0,0,.2,1)';ac.style.opacity='1';ac.style.transform='translateX(0)';function onFadeIn(){ac.removeEventListener('transitionend',onFadeIn);ac.style.transition='';ac.style.opacity='';ac.style.transform='';isSwitchingArticle=false}ac.addEventListener('transitionend',onFadeIn,{once:true})})}ac.addEventListener('transitionend',onFadeOut,{once:true})}
function closeArticleVisual(){if(!articleView.classList.contains('open'))return;articleView.classList.remove('open');toc.classList.remove('visible');closeTocSheet();unlockBody();currentArticleId=null;_lastFab=null;fabComment.classList.remove('visible');fabComment.classList.remove('shifted');fabComment.classList.remove('idle');clearTimeout(_fabIdleTimer);lastRouteWasArticle=false;requestAnimationFrame(function(){siteWrapper.scrollTop=scrollPos;topAppBar.classList.toggle('elevated',scrollPos>0);fab.classList.toggle('visible',scrollPos>400)})}
var _pageEls=null;
function switchPageVisual(name,dir){pageScrollPos[currentPage]=siteWrapper.scrollTop;if(currentPage==='articles')savedPaginationPage=paginationPage;syncTabs(name);if(!_pageEls)_pageEls=document.querySelectorAll('.page');for(var i=0;i<_pageEls.length;i++){_pageEls[i].classList.remove('active','slide-in-right','slide-in-left')}var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));if(el){if(dir&&!reducedMotion){var cls=dir==='right'?'slide-in-right':'slide-in-left';el.classList.add('active',cls);var onEnd=function(){el.classList.remove(cls);el.removeEventListener('animationend',onEnd)};el.addEventListener('animationend',onEnd)}else{el.classList.add('active')}setPageStagger('page'+name.charAt(0).toUpperCase()+name.slice(1))}if(name==='articles'){var all=getAllCards();for(var i=0;i<all.length;i++){all[i].dataset._filtered='';all[i].style.display='';all[i].classList.remove('show')}if(!skipClear){if(activeFilterLabel&&activeFilterFn){for(var i=0;i<all.length;i++){var c=all[i];if(!activeFilterFn(c)){c.dataset._filtered='1';c.style.display='none';c.classList.remove('show')}else{c.dataset._filtered='';c.style.display=''}}filterLabel.textContent=activeFilterLabel;filterBar.classList.add('on');paginationPage=0}else{filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=savedPaginationPage}}else{paginationPage=savedPaginationPage}renderPaginationPage()}currentPage=name;instantScroll(pageScrollPos[name]||0);if(name==='friends')__initFriendsTwikoo()}

var _artScrollRafPending=false;
articleView.addEventListener('scroll',function(){
  if(_artScrollRafPending)return;
  _artScrollRafPending=true;
  requestAnimationFrame(function(){
    _artScrollRafPending=false;
    // Batch all reads first
    var s=articleView.scrollTop,m=articleView.scrollHeight-articleView.clientHeight;
    // Batch all writes
    rprog.style.transform=m>0?'scaleX('+s/m+')':'scaleX(0)';
    var sc=s>0;
    if(sc!==_lastArtScrolled){avBar.classList.toggle('scrolled',sc);_lastArtScrolled=sc}
    var fv=s>400;
    if(fv!==_lastArtFab){fab.classList.toggle('visible',fv);fabComment.classList.toggle('shifted',fv);_lastArtFab=fv}
    clearTimeout(_fabIdleTimer);
    fabComment.classList.remove('idle');
    if(innerWidth<=768){
      _fabIdleTimer=setTimeout(function(){
        if(articleView.classList.contains('open')){fabComment.classList.add('idle')}
      },1000);
    }
    updateTOCHighlight()
  })
...
function handleMainScroll(){var y=siteWrapper.scrollTop,el=y>0;if(el!==_lastElevated){topAppBar.classList.toggle('elevated',el);_lastElevated=el}if(!articleView.classList.contains('open')){var fv=y>400;if(fv!==_lastFab){fab.classList.toggle('visible',fv);_lastFab=fv}}}
var _mainScrollPending=false;
siteWrapper.addEventListener('scroll',function(){if(_mainScrollPending)return;_mainScrollPending=true;requestAnimationFrame(function(){_mainScrollPending=false;handleMainScroll()})},{passive:true});
...
function instantScroll(y){siteWrapper.style.scrollBehavior='auto';siteWrapper.scrollTop=y;siteWrapper.style.scrollBehavior='';_lastElevated=null;_lastFab=null;handleMainScroll()}
...
function clearFilter(){activeFilterLabel='';activeFilterFn=null;filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=0;var all=getAllCards();for(var i=0;i<all.length;i++){all[i].dataset._filtered='';all[i].style.display='';all[i].classList.remove('show')}requestAnimationFrame(function(){renderPaginationPage()})}

function renderTags(){var el=$('tagsCount');el.textContent='共 '+allTags.length+' 个标签';var parts=[];for(var i=0;i<allTags.length;i++){var t=allTags[i];parts.push('<div class="tag-card" data-tag="'+t.n+'"><div class="tag-card-name">'+t.n+'</div><div class="tag-card-count">'+t.c+' 篇文章</div></div>')}$('tagsGrid').innerHTML=parts.join('')}
function renderCats(){var el=$('catsCount');el.textContent='共 '+cats.length+' 个分类';var parts=[];for(var i=0;i<cats.length;i++){var c=cats[i];parts.push('<div class="cat-card" data-cat="'+c.f+'"><div class="cat-dot" style="background:'+c.col+'"></div><div class="cat-name">'+c.n+'</div><div class="cat-desc">'+c.d+'</div><div class="cat-count">'+c.c+' 篇文章</div></div>')}$('catsGrid').innerHTML=parts.join('')}
function renderArchive(){var entries=[];for(var k=0;k<articleOrder.length;k++){var id=articleOrder[k];var art=articles[id];if(!art)continue;var m=art.dateISO.match(/(\d{4})-(\d{2})-(\d{2})/);if(!m)continue;entries.push({id:id,year:+m[1],month:+m[2],day:+m[3],title:art.title,tags:art.tags,rt:art.rt,category:art.category})}entries.sort(function(a,b){return(b.year*10000+b.month*100+b.day)-(a.year*10000+a.month*100+a.day)});var grouped={};for(var i=0;i<entries.length;i++){var e=entries[i];if(!grouped[e.year])grouped[e.year]=[];grouped[e.year].push(e)}var parts=[],idx=0;var monthNames={1:"Jan",2:"Feb",3:"Mar",4:"Apr",5:"May",6:"Jun",7:"Jul",8:"Aug",9:"Sep",10:"Oct",11:"Nov",12:"Dec"};var years=Object.keys(grouped).sort(function(a,b){return b-a});for(var yi=0;yi<years.length;yi++){var y=years[yi];var items=grouped[y];parts.push('<div class="archive-year-group"><div class="archive-year-label">'+y+'<span class="archive-year-count">'+items.length+' 篇</span></div>');for(var j=0;j<items.length;j++){var e=items[j];var tag=e.tags.length?e.tags[0]:'';parts.push('<div class="archive-card" data-id="'+e.id+'" style="--arch-d:'+(idx++*50)+'ms"><div class="archive-card-date"><span class="archive-card-month">'+monthNames[e.month]+'</span><span class="archive-card-day">'+e.day+'</span></div><div class="archive-card-body"><div class="archive-card-title">'+escHtml(e.title)+'</div><div class="archive-card-meta">'+(tag?'<span class="archive-card-tag">'+escHtml(tag)+'</span>':'')+'<span class="archive-card-rt">'+e.rt+'</span></div></div></div>')}parts.push('</div>')}$('archiveCount').textContent='共 '+entries.length+' 篇文章';$('archiveTimeline').innerHTML=parts.join('')}
function renderFriends(){var parts=[];for(var i=0;i<friends.length;i++){var f=friends[i];parts.push('<a class="fl-card" href="'+escHtml(f.u)+'" target="_blank" rel="noopener"><img class="fl-avatar" src="'+escHtml(f.a)+'" alt="'+escHtml(f.n)+'" width="48" height="48" loading="lazy"><div class="fl-info"><div class="fl-name">'+escHtml(f.n)+'</div><div class="fl-desc">'+escHtml(f.d)+'</div></div></a>')}$('flGrid').innerHTML=parts.join('')}
function createRipple(e,el){if(isLowEnd)return;if(el.childElementCount>3)return;var r=el.getBoundingClientRect(),s=Math.max(r.width,r.height)*2,sp=document.createElement('span');sp.className='ripple';sp.style.cssText='width:'+s+'px;height:'+s+'px;left:'+(e.clientX-r.left-s/2)+'px;top:'+(e.clientY-r.top-s/2)+'px';el.appendChild(sp);sp.addEventListener('animationend',function(){sp.remove()})}
function syncAllIcons(t){var wraps=document.querySelectorAll('.theme-wrap');for(var i=0;i<wraps.length;i++){var w=wraps[i],s=w.querySelector('.sun'),m=w.querySelector('.moon');if(t==='dark'){s.classList.add('off');m.classList.remove('off')}else{s.classList.remove('off');m.classList.add('off')}}}
...
document.addEventListener('click',function(e){
  ...
  if((t=e.target.closest('#artTags .tag'))&&t.dataset.tagname){var tn=t.dataset.tagname;activeFilterLabel=tn;activeFilterFn=function(c){var tags=(c.dataset.tags||'').split('\u001f');return tags.indexOf(tn)!==-1};closeArticleVisual();if(currentPage!=='articles'){switchPageVisual('articles','left')}pushRoute({type:'page',page:'articles'});applyFilterDirect(tn,activeFilterFn);siteWrapper.scrollTo({top:0,behavior:'smooth'});return}
  ...
  if(e.target.closest('#fab')){if(articleView.classList.contains('open'))articleView.scrollTo({top:0,behavior:'smooth'});else siteWrapper.scrollTo({top:0,behavior:'smooth'});return}
...
window.addEventListener('resize',function(){clearTimeout(resizeTimer);resizeTimer=setTimeout(function(){_tabCache=null;updateTabIndicator()},120)},{passive:true});
...
renderPaginationPage();handleMainScroll();requestAnimationFrame(updateTabIndicator);
var _ric=window.requestIdleCallback||function(cb){return setTimeout(cb,1)};
_ric(function(){renderTags();renderCats();renderArchive();renderFriends()});
