...
var _lastElevated=null,_lastFab=null,_lastArtScrolled=null,_lastArtFab=null;
...
function setTocHighlight(id){lastActiveId=id;tocList.querySelectorAll('.toc-item').forEach(function(it){it.classList.toggle('active',it.dataset.target===id)});tocDrawerList.querySelectorAll('.toc-drawer-item').forEach(function(it){it.classList.toggle('active',it.dataset.target===id)})}
...
function loadArticleMeta(id){var art=articles[id];if(!art)return;currentArticleId=id;$('artMeta').innerHTML='<span>'+art.date+'</span><span style="color:var(--outline-variant)"> · </span><span>'+art.wc+' 字</span><span style="color:var(--outline-variant)"> · </span><span>'+art.rt+'</span>';$('artTitle').textContent=art.title;var abt=$('avBarTitleText');abt.textContent=art.title;abt.classList.remove('scrolling');abt.style.transform='';abt.style.animationName='none';void abt.offsetWidth;var container=$('avBarTitle');if(abt.scrollWidth>container.clientWidth+4){var dx=-(abt.scrollWidth-container.clientWidth-4);abt.style.setProperty('--marquee-dx',dx+'px');abt.style.setProperty('--marquee-dur',Math.max(6,Math.abs(dx)/30)+'s');abt.style.animationName='';abt.classList.add('scrolling')}$('artTags').innerHTML=art.tags.map(function(t){return'<span class="tag" data-tagname="'+escHtml(t)+'">'+escHtml(t)+'</span>'}).join('');renderArticleNav(id);setTimeout(function(){__initTwikoo('/posts/'+id+'/')},100)}
function loadArticle(id){var art=articles[id];if(!art)return;loadArticleMeta(id);rprog.style.transform='scaleX(0)';fab.classList.remove('visible');articleBody.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--outline)">加载中…</div>';articleView.scrollTop=0;headingCache=null;lastActiveId=null;tocManualLock=false;clearTimeout(tocManualTimer);_lastArtScrolled=null;_lastArtFab=null;function onContent(html){articleBody.innerHTML=html;codeProcessed=false;processCodeBlocks();applyStagger();revealImages();observeImages();articleBody.classList.remove('revealed');if(!isLowEnd){void articleBody.offsetHeight;requestAnimationFrame(function(){articleBody.classList.add('revealed')})}else{articleBody.classList.add('revealed')}setTimeout(buildTOC,80)}if(__articleCache[id]){onContent(__articleCache[id]);return}fetch('/posts/'+id+'/article.json').then(function(r){return r.json()}).then(function(d){__articleCache[id]=d.content;onContent(d.content)}).catch(function(){articleBody.innerHTML='<div style="text-align:center;padding:60px 0;color:var(--outline)">加载失败，请刷新重试</div>'})}
function openArticleVisual(id){scrollPos=window.scrollY;loadArticle(id);lockBody();void articleBody.offsetHeight;articleView.classList.add('open');avBar.classList.remove('scrolled')}
function openArticleVisualImmediate(id){loadArticle(id);lockBody();articleView.style.transition='none';articleView.classList.add('open');articleView.offsetHeight;articleView.style.transition='';avBar.classList.remove('scrolled')}
function switchToArticleVisual(nid,dir){if(isSwitchingArticle)return;isSwitchingArticle=true;var ac=$('avContent'),dx=dir==='next'?-16:16;ac.style.transition='opacity 160ms cubic-bezier(.4,0,1,1),transform 160ms cubic-bezier(.4,0,1,1)';ac.style.opacity='0';ac.style.transform='translateX('+dx+'px)';setTimeout(function(){ac.style.transition='none';ac.style.transform='translateX('+(-dx)+'px)';loadArticle(nid);void ac.offsetHeight;ac.style.transition='opacity 240ms cubic-bezier(0,0,.2,1),transform 240ms cubic-bezier(0,0,.2,1)';ac.style.opacity='1';ac.style.transform='translateX(0)';setTimeout(function(){ac.style.transition='';ac.style.opacity='';ac.style.transform='';isSwitchingArticle=false},250)},170)}
function closeArticleVisual(){if(!articleView.classList.contains('open'))return;articleView.classList.remove('open');toc.classList.remove('visible');closeTocSheet();unlockBody();currentArticleId=null;_lastFab=null;lastRouteWasArticle=false;$('avBarTitleText').textContent='Moriefy';$('avBarTitleText').classList.remove('scrolling');$('avBarTitleText').style.transform='';requestAnimationFrame(function(){window.scrollTo(0,scrollPos);topAppBar.classList.toggle('elevated',scrollPos>0);fab.classList.toggle('visible',scrollPos>400)})}
function switchPageVisual(name,dir){pageScrollPos[currentPage]=window.scrollY;if(currentPage==='articles')savedPaginationPage=paginationPage;syncTabs(name);document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active','slide-in-right','slide-in-left')});var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));if(el){if(dir&&!reducedMotion){var cls=dir==='right'?'slide-in-right':'slide-in-left';el.classList.add('active',cls);var onEnd=function(){el.classList.remove(cls);el.removeEventListener('animationend',onEnd)};el.addEventListener('animationend',onEnd)}else{el.classList.add('active')}setPageStagger('page'+name.charAt(0).toUpperCase()+name.slice(1))}if(name==='articles'){renderAllCards();if(!skipClear){if(activeFilterLabel&&activeFilterFn){getAllCards().forEach(function(c){if(!activeFilterFn(c)){c.dataset._filtered='1';c.style.display='none';c.classList.remove('show')}else{c.dataset._filtered='';c.style.display=''}});filterLabel.textContent=activeFilterLabel;filterBar.classList.add('on');paginationPage=0}else{filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=savedPaginationPage}}else{paginationPage=savedPaginationPage}renderPaginationPage()}currentPage=name;instantScroll(pageScrollPos[name]||0)}

articleView.addEventListener('scroll',function(){if(artScrollRaf)return;artScrollRaf=requestAnimationFrame(function(){artScrollRaf=null;var s=articleView.scrollTop,m=articleView.scrollHeight-articleView.clientHeight;rprog.style.transform=m>0?'scaleX('+s/m+')':'scaleX(0)';var sc=s>0;if(sc!==_lastArtScrolled){avBar.classList.toggle('scrolled',sc);_lastArtScrolled=sc}var fv=s>400;if(fv!==_lastArtFab){fab.classList.toggle('visible',fv);_lastArtFab=fv}updateTOCHighlight()})},{passive:true});
...
function updateTabIndicator(){var a=document.querySelector('.tab.active');if(a&&tabBar){tabBar.style.transform='translateX('+a.offsetLeft+'px) scaleX('+a.offsetWidth+')'}}
...
function clearFilter(){activeFilterLabel='';activeFilterFn=null;filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=0;var all=getAllCards();all.forEach(function(c){c.dataset._filtered='';c.style.display='';c.classList.remove('show')});void document.body.offsetHeight;requestAnimationFrame(function(){renderPaginationPage()})}
