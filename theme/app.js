...
function __initFriendsComments(){var _c=window.__HAPRIAL_DATA__.comments;if(!_c||!_c.enabled)return;var sec=document.getElementById('friendsCommentSection');if(!sec||sec.dataset.loaded)return;sec.dataset.loaded='1';var el=document.getElementById('tcomment');if(!el||typeof window.__initComments!=='function')return;window.__initComments()}
...
function setArticleFAB(){var s=articleView.scrollTop,vis=s>400;fab.classList.toggle('visible',vis);fabComment.classList.add('visible');fabComment.classList.toggle('shifted',vis);_lastArtFab=vis;_lastArtScrolled=s>0}
function loadArticle(id){var art=articles[id];if(!art)return;loadArticleMeta(id);rprog.style.transform='scaleX(0)';articleView.scrollTop=0;clearTimeout(tocBuildTimer);_lastArtScrolled=null;_lastArtFab=null;articleBody.classList.remove('revealed');function renderContent(html){articleBody.innerHTML=html;_artScrollMax=articleView.scrollHeight-articleView.clientHeight;codeProcessed=false;processCodeBlocks();revealImages();observeImages();requestAnimationFrame(function(){articleBody.classList.add('revealed')});tocBuildTimer=setTimeout(buildTOC,80);processMermaid();var _cm=window.__HAPRIAL_DATA__.comments;if(_cm&&_cm.enabled&&typeof window.__initComments==='function'){window.__initComments()}setTimeout(function(){prefetchNeighbors(id)},300);setArticleFAB()}if(_contentCache[id]){renderContent(_contentCache[id]);var ci=_cacheOrder.indexOf(id);if(ci>0){_cacheOrder.splice(ci,1);_cacheOrder.push(id)}return}articleBody.innerHTML='<div class="skel-wrap"><div class="skel-line skel-w60"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w90"></div><div class="skel-line skel-w70"></div><div class="skel-gap"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w80"></div><div class="skel-line skel-w95"></div><div class="skel-line skel-w60"></div><div class="skel-gap"></div><div class="skel-line skel-w100"></div><div class="skel-line skel-w75"></div><div class="skel-line skel-w85"></div></div>';fetch('/posts/'+id+'/article.json').then(function(r){if(!r.ok)throw new Error(r.status);return r.json()}).then(function(d){cachePut(id,d.content||'');renderContent(_contentCache[id])}).catch(function(){articleBody.innerHTML='<p style="text-align:center;padding:80px 0;color:var(--outline)">加载失败，请刷新重试。</p>'})}
function openArticleVisual(id){scrollPos=window.scrollY;loadArticle(id);lockBody();requestAnimationFrame(function(){articleView.classList.add('open')});avBar.classList.remove('scrolled')}
function openArticleVisualImmediate(id){loadArticle(id);lockBody();articleView.style.transition='none';articleView.classList.add('open');articleView.offsetHeight;requestAnimationFrame(function(){articleView.style.transition=''});avBar.classList.remove('scrolled');_lastFab=null}
...
new MutationObserver(function(){if(articleView.classList.contains('open'))setArticleFAB()}).observe(articleView,{attributes:true,attributeFilter:['class']});
