...
articleView.addEventListener('scroll',function(){if(artScrollRaf)return;artScrollRaf=requestAnimationFrame(function(){artScrollRaf=null;var s=articleView.scrollTop,m=articleView.scrollHeight-articleView.clientHeight;rprog.style.transform=m>0?'scaleX('+s/m+')':'scaleX(0)';var sc=s>0;if(sc!==_lastArtScrolled){avBar.classList.toggle('scrolled',sc);_lastArtScrolled=sc}var fv=s>400;if(fv!==_lastArtFab){fab.classList.toggle('visible',fv);fabComment.classList.toggle('visible',fv);_lastArtFab=fv}updateTOCHighlight()})},{passive:true});
