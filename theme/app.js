...
function setTocHighlight(id){lastActiveId=id;var activeItem=null,activeDrawerItem=null;var items=tocList.children;for(var i=0;i<items.length;i++){var it=items[i];it.classList.toggle('active',it.dataset.target===id);if(it.dataset.target===id)activeItem=it}var dItems=tocDrawerList.children;for(var i=0;i<dItems.length;i++){var di=dItems[i];di.classList.toggle('active',di.dataset.target===id);if(di.dataset.target===id)activeDrawerItem=di}if(activeItem){var aTop=activeItem.offsetTop,sTop=toc.scrollTop,cH=toc.clientHeight;if(aTop<sTop||aTop+activeItem.offsetHeight>sTop+cH){toc.scrollTop=aTop-cH/2}}if(activeDrawerItem&&tocSheet.classList.contains('open')){var dTop=activeDrawerItem.offsetTop,dsTop=tocDrawerList.scrollTop,dH=tocDrawerList.clientHeight;if(dTop<dsTop||dTop+activeDrawerItem.offsetHeight>dsTop+dH){tocDrawerList.scrollTop=dTop-dH/3}}}
...
window.addEventListener('resize',function(){clearTimeout(resizeTimer);resizeTimer=setTimeout(function(){updateTabIndicator();sizeTocHoverZone()},120)});
...
// ── TOC Hover Zone ──
function sizeTocHoverZone(){
  if(!tocHoverZone||!articleView)return;
  var av=document.querySelector('.av-content');
  if(!av)return;
  var rect=av.getBoundingClientRect();
  var w=Math.max(window.innerWidth-rect.right,230);
  tocHoverZone.style.width=w+'px';
}
if(tocHoverZone){
  tocHoverZone.addEventListener('mouseenter',function(){
    clearTimeout(tocHideTimer);
    toc.classList.add('visible');
  });
  tocHoverZone.addEventListener('mouseleave',function(){
    clearTimeout(tocHideTimer);
