...
function openArticleVisual(id){scrollPos=window.scrollY;articleView.classList.add('open');requestAnimationFrame(function(){lockBody();loadArticle(id);avBar.classList.remove('scrolled');fabComment.classList.add('visible');fabComment.classList.remove('shifted');var af=document.getElementById('anti-fouc');if(af)af.remove()})}
