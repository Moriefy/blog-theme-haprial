...
function switchToArticleVisual(nid,dir){
  ...
  var dF=parseInt(cs.getPropertyValue('--dur-fast'))||150;
  var dM=parseInt(cs.getPropertyValue('--dur-med'))||200;
  function finish(){isSwitchingArticle=false;ac.style.transition='';ac.style.opacity='';ac.style.transform=''}
  /* MD3 shared-axis: exit scales+fades down, incoming scales+fades up in place — no lateral translate */
  ac.style.transition='opacity '+dF+'ms var(--motion-standard),transform '+dF+'ms var(--motion-standard)';
  ac.style.opacity='0';ac.style.transform='scale(.985)';
  var done1=false;
  ...
  setTimeout(function(){if(!done1){done1=true;doLoad()}},dF+20);
  function doLoad(){
    ac.style.transition='none';ac.style.transform='scale(1)';ac.style.opacity='0';
    ...
    requestAnimationFrame(function(){
      ac.style.transition='opacity '+dM+'ms var(--motion-emphasized-decelerate),transform '+dM+'ms var(--motion-emphasized-decelerate)';
      ac.style.opacity='1';ac.style.transform='scale(1)';
      var done2=false;
      ac.addEventListener('transitionend',function h2(){if(done2)return;done2=true;finish()},{once:true});
      setTimeout(function(){if(!done2){done2=true;finish()}},dM+20);
    });
  }
}
...
function switchPageVisual(name,dir){pageScrollPos[currentPage]=window.scrollY;var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));var curEl=$('page'+currentPage.charAt(0).toUpperCase()+currentPage.slice(1));function doSwitch(){syncTabs(name);if(curEl)curEl.classList.remove('active','slide-in-right','slide-in-left','slide-in','slide-out-left','slide-out-right','slide-out');if(el){var inCls=isMobile? 'slide-in':(dir? (dir==='right'?'slide-in-right':'slide-in-left') : 'slide-in');el.classList.add('active',inCls);el.addEventListener('animationend',function(){el.classList.remove('slide-in','slide-in-right','slide-in-left')},{once:true})}currentPage=name;if(name==='articles'&&activeFilterLabel){applyFilterDirect(activeFilterLabel,activeFilterFn)}if(name==='friends')__initFriendsComments();instantScroll(pageScrollPos[name]||0)}if(dir&&!reducedMotion&&curEl&&curEl!==el){var exitCls=isMobile? 'slide-out':(dir==='right'?'slide-out-left':'slide-out-right');var done=false;curEl.classList.add(exitCls);curEl.addEventListener('animationend',function(){if(done)return;done=true;curEl.classList.remove(exitCls);doSwitch()},{once:true});setTimeout(function(){if(!done){done=true;curEl.classList.remove(exitCls);doSwitch()}},220)}else{doSwitch()}}
...
function filterSearch(q){q=(q||'').toLowerCase().trim();if(!q){svResults.innerHTML='<p class="sv-hint">输入关键词搜索文章、标签和摘要。</p>';return}var cards=getAllCards(),html='',found=false,si=0;cards.forEach(function(c){var t=c.dataset.title||'',e=c.dataset.excerpt||'',g=c.dataset.tags||'';if(t.toLowerCase().indexOf(q)!==-1||e.toLowerCase().indexOf(q)!==-1||g.toLowerCase().indexOf(q)!==-1){found=true;var tag=c.querySelector('.tag');html+='<div class="sv-item" style="--sd:"+(si++)*30+"ms" data-id="'+c.dataset.id+'"><div class="sv-item-tag">'+(tag?tag.textContent:'')+'</div><div class="sv-item-title">'+hlMatch(t,q)+'</div><div class="sv-item-excerpt">'+hlMatch(e,q)+'</div></div>'}});svResults.innerHTML=found?html:'<div class="sv-no-results">未找到相关结果。</div>';searchActiveIdx=-1}
...
document.addEventListener('pointerdown',function(e){var el=e.target.closest('.fl-card,.art-nav-btn,.archive-card,#filterClear,.pg-btn,.tab,.icon-btn,.sv-item,.about-link,.footer-link,.about-skill,.filter-clear,.tag,.cb-copy');if(!el||el.disabled||el.offsetParent===null)return;createRipple(e,el)},{passive:true});
