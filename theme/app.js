...
function updateTOCHighlight(){if(tocManualLock||!headingCache||!headingCache.length)return;if(_headingCacheDirty)cacheHeadings();var st=articleView.scrollTop,viewH=articleView.clientHeight,vTop=st,vBot=st+viewH;if(viewH<1)return;var best=null,bestVis=0;for(var i=0;i<headingCache.length;i++){var secTop=headingCache[i].top,secBot=i+1<headingCache.length?headingCache[i+1].top:articleView.scrollHeight;var vis=Math.max(0,Math.min(secBot,vBot)-Math.max(secTop,vTop));if(vis>bestVis){bestVis=vis;best=headingCache[i]}}if(best&&best.id!==lastActiveId)setTocHighlight(best.id)}
