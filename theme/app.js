...
function switchPageVisual(name,dir){pageScrollPos[currentPage]=window.scrollY;if(currentPage==='articles')savedPaginationPage=paginationPage;var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));var doSwitch=function(){syncTabs(name);document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active','slide-in-right','slide-in-left')});if(el){if(dir&&!reducedMotion){var cls=dir==='right'?'slide-in-right':'slide-in-left';el.classList.add('active',cls);var onEnd=function(){el.classList.remove(cls);el.removeEventListener('animationend',onEnd)};el.addEventListener('animationend',onEnd)}else{el.classList.add('active')}}currentPage=name;instantScroll(pageScrollPos[name]||0);if(name==='friends')__initFriendsTwikoo()};setTimeout(doSwitch,0)}
...
function clearFilter(){activeFilterLabel='';activeFilterFn=null;filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=0;getAllCards().forEach(function(c){c.dataset._filtered='';c.style.display='none';c.classList.remove('show')});setTimeout(function(){renderPaginationPage()},0)}
