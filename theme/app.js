...
function createRipple(e,el){if(isLowEnd)return;if(el.querySelectorAll('.ripple:not(.fade-out)').length>2)return;var r=el.getBoundingClientRect(),s=Math.max(r.width,r.height)*2,sp=document.createElement('span');sp.className='ripple';sp.style.cssText='width:'+s+'px;height:'+s+'px;left:'+(e.clientX-r.left-s/2)+'px;top:'+(e.clientY-r.top-s/2)+'px';el.appendChild(sp);function fade(){document.removeEventListener('pointerup',fade);document.removeEventListener('pointercancel',fade);sp.classList.add('fade-out');sp.addEventListener('animationend',function(){sp.remove()})}document.addEventListener('pointerup',fade);document.addEventListener('pointercancel',fade)}
