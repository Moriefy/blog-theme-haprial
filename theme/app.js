...
function prefetchArticle(id){if(_contentCache[id]||_prefetchInFlight[id])return;_prefetchInFlight[id]=true;fetch('/posts/'+id+'/article.json').then(function(r){if(!r.ok)throw new Error(r.status);return r.json()}).then(function(d){_contentCache[id]=d.content||''}).catch(function(){}).finally(function(){delete _prefetchInFlight[id]})}
...
function switchPageVisual(name,dir){pageScrollPos[currentPage]=window.scrollY;if(currentPage==='articles')savedPaginationPage=paginationPage;var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));var curEl=$('page'+currentPage.charAt(0).toUpperCase()+currentPage.slice(1));var doSwitch=function(){syncTabs(name);document.querySelectorAll('.page').forEach(function(p){p.classList.remove('active','slide-in-right','slide-in-left','slide-out-left','slide-out-right')});if(el){if(dir&&!reducedMotion){var cls=dir==='right'?'slide-in-right':'slide-in-left';el.classList.add('active',cls);var onEnd=function(){el.classList.remove(cls);el.removeEventListener('animationend',onEnd)};el.addEventListener('animationend',onEnd)}else{el.classList.add('active')}}currentPage=name;instantScroll(pageScrollPos[name]||0);if(name==='friends')__initFriendsTwikoo()};if(dir&&!reducedMotion&&curEl&&curEl!==el){var exitCls=dir==='right'?'slide-out-left':'slide-out-right';curEl.classList.add(exitCls);var exitEnd=function(){curEl.classList.remove(exitCls);curEl.removeEventListener('animationend',exitEnd);doSwitch()};curEl.addEventListener('animationend',exitEnd);setTimeout(doSwitch,160)}else{setTimeout(doSwitch,0)}}

var _artScrollThrottle=null;
articleView.addEventListener('scroll',function(){if(_artScrollThrottle)return;_artScrollThrottle=setTimeout(function(){_artScrollThrottle=null;if(artScrollRaf)return;artScrollRaf=requestAnimationFrame(function(){artScrollRaf=null;var s=articleView.scrollTop,m=articleView.scrollHeight-articleView.clientHeight,sc=s>0,fv=s>400,needIdle=innerWidth<=768&&articleView.classList.contains('open');rprog.style.transform=m>0?'scaleX('+s/m+')':'scaleX(0)';if(sc!==_lastArtScrolled){avBar.classList.toggle('scrolled',sc);_lastArtScrolled=sc}if(fv!==_lastArtFab){fab.classList.toggle('visible',fv);fabComment.classList.toggle('shifted',fv);_lastArtFab=fv}if(needIdle){clearTimeout(_fabIdleTimer);fab.classList.remove('idle');fabComment.classList.remove('idle');_fabIdleTimer=setTimeout(function(){if(articleView.classList.contains('open')){fab.classList.add('idle');fabComment.classList.add('idle')}},1000)}updateTOCHighlight()})},100)},{passive:true});
...
function clearFilter(){activeFilterLabel='';activeFilterFn=null;if(!reducedMotion&&filterBar.classList.contains('on')){filterBar.classList.add('out');filterBar.classList.remove('on');filterBar.addEventListener('animationend',function handler(){filterBar.removeEventListener('animationend',handler);filterBar.classList.remove('out');filterBar.style.display='none'},{once:true})}else{filterBar.classList.remove('on');filterBar.style.display='none'}filterLabel.textContent='';paginationPage=0;getAllCards().forEach(function(c){c.dataset._filtered='';c.style.display='none';c.classList.remove('show')});setTimeout(function(){renderPaginationPage()},0)}
