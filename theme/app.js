...
function setTocHighlight(id){lastActiveId=id;var activeItem=null;tocList.querySelectorAll('.toc-item').forEach(function(it){var isActive=it.dataset.target===id;it.classList.toggle('active',isActive);if(isActive)activeItem=it});tocDrawerList.querySelectorAll('.toc-drawer-item').forEach(function(it){it.classList.toggle('active',it.dataset.target===id)});if(activeItem){var rect=activeItem.getBoundingClientRect(),container=toc;if(rect.top<container.getBoundingClientRect().top||rect.bottom>container.getBoundingClientRect().bottom){activeItem.scrollIntoView({block:'nearest',behavior:'instant'})}}}
...
function switchPageVisual(name,dir){pageScrollPos[currentPage]=window.scrollY;if(currentPage==='articles')savedPaginationPage=paginationPage;syncTabs(name);if(!_pageEls)_pageEls=document.querySelectorAll('.page');for(var i=0;i<_pageEls.length;i++){_pageEls[i].classList.remove('active','slide-in-right','slide-in-left')}var el=$('page'+name.charAt(0).toUpperCase()+name.slice(1));if(el){el.classList.add('active');if(dir&&!reducedMotion){el.classList.add(dir==='right'?'slide-in-right':'slide-in-left')}}if(name==='articles'){var all=getAllCards();for(var i=0;i<all.length;i++){var c=all[i];c.dataset._filtered='';c.style.display='';c.classList.remove('show')}if(!skipClear&&activeFilterLabel&&activeFilterFn){for(var i=0;i<all.length;i++){if(!activeFilterFn(all[i])){all[i].dataset._filtered='1';all[i].style.display='none'}}filterLabel.textContent=activeFilterLabel;filterBar.classList.add('on');paginationPage=0}else{filterBar.classList.remove('on');filterLabel.textContent='';paginationPage=savedPaginationPage}renderPaginationPage()}currentPage=name;instantScroll(pageScrollPos[name]||0);if(name==='friends')__initFriendsTwikoo()}
...
function updateTabIndicator(){var a=document.querySelector('.tab.active');if(a&&tabBar){tabBar.style.width=a.offsetWidth+'px';tabBar.style.transform='translateX('+a.offsetLeft+'px)'}}
