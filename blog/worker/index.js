...
async function ensureTables(db){
  ...
  await db.batch([
    ...
    // 新功能表
    db.prepare(`CREATE TABLE IF NOT EXISTS site_stats(id INTEGER PRIMARY KEY AUTOINCREMENT,page_slug TEXT NOT NULL,visits INTEGER DEFAULT 0,date TEXT NOT NULL,created_at TEXT DEFAULT (datetime('now')),updated_at TEXT DEFAULT (datetime('now')),UNIQUE(page_slug, date))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS seo_config(id INTEGER PRIMARY KEY AUTOINCREMENT,page_slug TEXT UNIQUE,meta_title TEXT,meta_description TEXT,og_image TEXT,canonical_url TEXT,robots TEXT DEFAULT 'index, follow',created_at TEXT DEFAULT (datetime('now')),updated_at TEXT DEFAULT (datetime('now')))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS performance_logs(id INTEGER PRIMARY KEY AUTOINCREMENT,metric TEXT NOT NULL,value REAL NOT NULL,timestamp TEXT DEFAULT (datetime('now')),page_slug TEXT,user_agent TEXT,ip TEXT)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS article_versions(id INTEGER PRIMARY KEY AUTOINCREMENT,article_id INTEGER NOT NULL,version INTEGER NOT NULL,title TEXT,content TEXT,created_at TEXT DEFAULT (datetime('now')),FOREIGN KEY (article_id) REFERENCES articles(id) ON DELETE CASCADE)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS backups(id INTEGER PRIMARY KEY AUTOINCREMENT,filename TEXT NOT NULL,size INTEGER,created_at TEXT DEFAULT (datetime('now')),type TEXT DEFAULT 'manual',status TEXT DEFAULT 'completed')`),
    db.prepare(`CREATE TABLE IF NOT EXISTS theme_config(id INTEGER PRIMARY KEY AUTOINCREMENT,key TEXT UNIQUE,value TEXT,created_at TEXT DEFAULT (datetime('now')),updated_at TEXT DEFAULT (datetime('now')))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS integrations(id INTEGER PRIMARY KEY AUTOINCREMENT,service TEXT UNIQUE,config TEXT,enabled INTEGER DEFAULT 1,created_at TEXT DEFAULT (datetime('now')),updated_at TEXT DEFAULT (datetime('now')))`),
    db.prepare(`CREATE TABLE IF NOT EXISTS push_subscriptions(id INTEGER PRIMARY KEY AUTOINCREMENT,endpoint TEXT UNIQUE,keys TEXT,user_agent TEXT,created_at TEXT DEFAULT (datetime('now')))`),
  ]);
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path.startsWith('/api/admin/')) {
        ...
        // ── 数据分析与统计 ──
        // GET /api/admin/analytics - 获取详细统计数据
        if (method === 'GET' && path === '/api/admin/analytics') {
          // 获取最近30天的访问统计
          const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
          const visits = await env.DB.prepare("SELECT page_slug, SUM(visits) as total_visits FROM site_stats WHERE date >= ? GROUP BY page_slug ORDER BY total_visits DESC").bind(thirtyDaysAgo).all();
          // 获取热门文章（按访问量排序）
          const topArticles = await env.DB.prepare("SELECT a.id, a.title, a.slug, COALESCE(s.visits, 0) as visits FROM articles a LEFT JOIN (SELECT page_slug, SUM(visits) as visits FROM site_stats WHERE date >= ? GROUP BY page_slug) s ON a.slug = s.page_slug WHERE a.status = 'published' ORDER BY s.visits DESC LIMIT 10").bind(thirtyDaysAgo).all();
          // 获取每日访问趋势
          const dailyVisits = await env.DB.prepare("SELECT date, SUM(visits) as total_visits FROM site_stats WHERE date >= ? GROUP BY date ORDER BY date").bind(thirtyDaysAgo).all();
          return json({ visits: visits.results, topArticles: topArticles.results, dailyVisits: dailyVisits.results });
        }

        // POST /api/admin/analytics/record - 记录页面访问
        if (method === 'POST' && path === '/api/admin/analytics/record') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { page_slug } = body;
          if (!page_slug) return json({ error: '缺少页面标识' }, 400);
          const today = new Date().toISOString().split('T')[0];
          await env.DB.prepare("INSERT INTO site_stats (page_slug, visits, date) VALUES (?, 1, ?) ON CONFLICT(page_slug, date) DO UPDATE SET visits = visits + 1, updated_at = datetime('now')").bind(page_slug, today).run();
          return json({ ok: true });
        }

        // ── SEO优化工具 ──
        // GET /api/admin/seo - 获取所有页面的SEO配置
        if (method === 'GET' && path === '/api/admin/seo') {
          const { results } = await env.DB.prepare("SELECT * FROM seo_config ORDER BY page_slug").all();
          return json({ seo: results });
        }

        // PUT /api/admin/seo/:slug - 更新特定页面的SEO配置
        if (method === 'PUT' && /^\/api\/admin\/seo\/[^\/]+$/.test(path)) {
          const slug = path.split('/').pop();
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { meta_title, meta_description, og_image, canonical_url, robots } = body;
          await env.DB.prepare("INSERT INTO seo_config (page_slug, meta_title, meta_description, og_image, canonical_url, robots) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(page_slug) DO UPDATE SET meta_title=excluded.meta_title, meta_description=excluded.meta_description, og_image=excluded.og_image, canonical_url=excluded.canonical_url, robots=excluded.robots, updated_at=datetime('now')").bind(slug, meta_title || null, meta_description || null, og_image || null, canonical_url || null, robots || 'index, follow').run();
          return json({ ok: true });
        }

        // GET /api/admin/seo/sitemap - 生成Sitemap
        if (method === 'GET' && path === '/api/admin/seo/sitemap') {
          const articles = await env.DB.prepare("SELECT slug, updated_at FROM articles WHERE status='published' ORDER BY updated_at DESC").all();
          const siteConfig = await env.DB.prepare("SELECT value FROM admin_config WHERE key='site_url'").first();
          const siteUrl = siteConfig ? siteConfig.value : 'https://example.com';
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url>
    ...
    <lastmod>${article.updated_at.split(' ')[0]}</lastmod>
    <changefreq>monthly</changefreq>
    <priority>0.8</priority>
  </url>`;
          }
          sitemap += '\n</urlset>';
          return new Response(sitemap, { headers: { ...cors, 'Content-Type': 'application/xml' } });
        }

        // DELETE /api/admin/seo/:slug - 删除特定页面的SEO配置
        if (method === 'DELETE' && /^\/api\/admin\/seo\/[^\/]+$/.test(path)) {
          const slug = path.split('/').pop();
          await env.DB.prepare("DELETE FROM seo_config WHERE page_slug=?").bind(slug).run();
          return json({ ok: true });
        }

        // ── 性能监控 ──
        // POST /api/admin/performance/record - 记录性能指标
        if (method === 'POST' && path === '/api/admin/performance/record') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { metric, value, page_slug, user_agent, ip } = body;
          if (!metric || value === undefined) return json({ error: '缺少metric或value' }, 400);
          await env.DB.prepare("INSERT INTO performance_logs (metric, value, page_slug, user_agent, ip) VALUES (?, ?, ?, ?, ?)").bind(metric, value, page_slug || null, user_agent || null, ip || null).run();
          return json({ ok: true });
        }

        // GET /api/admin/performance - 获取性能统计
        if (method === 'GET' && path === '/api/admin/performance') {
          const { results: logs } = await env.DB.prepare(`
            SELECT metric, AVG(value) as avg_value, MIN(value) as min_value, MAX(value) as max_value, COUNT(*) as count
            FROM performance_logs 
            WHERE timestamp >= datetime('now', '-7 days')
            GROUP BY metric
            ORDER BY metric
          `).all();
          return json({ performance: logs });
        }

        // GET /api/admin/performance/recent - 获取最近性能记录
        if (method === 'GET' && path === '/api/admin/performance/recent') {
        ...
        if (method === 'GET' && /^\/api\/admin\/articles\/\d+\/versions$/.test(path)) {
        ...
        if (method === 'POST' && /^\/api\/admin\/articles\/\d+\/versions$/.test(path)) {
          const articleId = path.split('/')[4];
          // 获取当前文章内容
          ...
          // 获取最新版本号
          const lastVersion = await env.DB.prepare("SELECT MAX(version) as max_version FROM article_versions WHERE article_id=?").bind(articleId).first();
          const newVersion = (lastVersion.max_version || 0) + 1;
          
          // 保存版本
          await env.DB.prepare("INSERT INTO article_versions (article_id, version, title, content) VALUES (?, ?, ?, ?)").bind(articleId, newVersion, article.title, article.content).run();
          return json({ ok: true, version: newVersion });
        }

        // POST /api/admin/articles/:id/restore/:version - 恢复文章版本
        if (method === 'POST' && /^\/api\/admin\/articles\/\d+\/restore\/\d+$/.test(path)) {
          const parts = path.split('/');
          const articleId = parts[4];
          const version = parts[6];
          
          // 获取版本内容
          const versionData = await env.DB.prepare("SELECT * FROM article_versions WHERE article_id=? AND version=?").bind(articleId, version).first();
          if (!versionData) return json({ error: '版本不存在' }, 404);
          
          // 更新文章
          await env.DB.prepare("UPDATE articles SET title=?, content=?, updated_at=datetime('now') WHERE id=?").bind(versionData.title, versionData.content, articleId).run();
          return json({ ok: true });
        }

        // ── 自动化与备份 ──
        // POST /api/admin/backups - 创建备份
        if (method === 'POST' && path === '/api/admin/backups') {
          const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
          const filename = `backup-${timestamp}.json`;
          
          // 收集所有数据
          const articles = await env.DB.prepare("SELECT * FROM articles").all();
          const friends = await env.DB.prepare("SELECT * FROM friends").all();
          const seo = await env.DB.prepare("SELECT * FROM seo_config").all();
          const theme = await env.DB.prepare("SELECT * FROM theme_config").all();
          const integrations = await env.DB.prepare("SELECT * FROM integrations").all();
          ...
          // 保存备份记录
          await env.DB.prepare("INSERT INTO backups (filename, size, type) VALUES (?, ?, 'manual')").bind(filename, size).run();
          
          // 将备份数据推送到GitHub（可选）
          try {
            await githubPush(env, `backups/${filename}`, JSON.stringify(backupData, null, 2), `chore(backup): create backup ${filename}`);
          } catch (e) {
            console.error('备份推送到GitHub失败:', e);
          }
          
          return json({ ok: true, filename, size });
        }

        // GET /api/admin/backups - 获取备份列表
        if (method === 'GET' && path === '/api/admin/backups') {
          const { results: backups } = await env.DB.prepare("SELECT * FROM backups ORDER BY created_at DESC").all();
          return json({ backups });
        }

        // POST /api/admin/backups/:id/restore - 从备份恢复
        if (method === 'POST' && /^\/api\/admin\/backups\/\d+\/restore$/.test(path)) {
          const backupId = path.split('/')[4];
          
          // 获取备份记录
          const backup = await env.DB.prepare("SELECT * FROM backups WHERE id=?").bind(backupId).first();
          if (!backup) return json({ error: '备份不存在' }, 404);
          
          // 从GitHub获取备份文件
          const backupData = await githubFetch(env, `backups/${backup.filename}`);
          if (!backupData) return json({ error: '无法获取备份文件' }, 500);
          
          try {
            const data = JSON.parse(backupData.content);
            
            // 恢复数据（这里简化处理，实际应该更谨慎）
            if (data.articles) {
              await env.DB.prepare("DELETE FROM articles").run();
              for (const article of data.articles) {
                await env.DB.prepare("INSERT INTO articles (id, slug, title, date, tags, category, excerpt, content, status, pinned, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(article.id, article.slug, article.title, article.date, article.tags, article.category, article.excerpt, article.content, article.status, article.pinned, article.created_at, article.updated_at).run();
              }
            }
            
            return json({ ok: true });
          } catch (e) {
            return json({ error: '备份文件解析失败: ' + e.message }, 500);
          }
        }

        // ── 外观与主题自定义 ──
        // GET /api/admin/theme - 获取主题配置
        if (method === 'GET' && path === '/api/admin/theme') {
          const { results: theme } = await env.DB.prepare("SELECT * FROM theme_config ORDER BY key").all();
          return json({ theme });
        }

        // PUT /api/admin/theme - 更新主题配置
        if (method === 'PUT' && path === '/api/admin/theme') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { settings } = body;
          if (!settings || typeof settings !== 'object') return json({ error: '缺少设置数据' }, 400);
          
          // 批量更新设置
          for (const [key, value] of Object.entries(settings)) {
            await env.DB.prepare("INSERT INTO theme_config (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=datetime('now')").bind(key, value).run();
          }
          return json({ ok: true });
        }

        // POST /api/admin/theme/reset - 重置主题到默认
        if (method === 'POST' && path === '/api/admin/theme/reset') {
          await env.DB.prepare("DELETE FROM theme_config").run();
          // 可以在这里插入默认主题设置
          return json({ ok: true });
        }
        ...
        if (method === 'GET' && path === '/api/admin/integrations') {
          const { results: integrations } = await env.DB.prepare("SELECT * FROM integrations ORDER BY service").all();
          return json({ integrations });
        }

        // PUT /api/admin/integrations/:service - 更新集成配置
        if (method === 'PUT' && /^\/api\/admin\/integrations\/[^\/]+$/.test(path)) {
          const service = path.split('/').pop();
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { config, enabled } = body;
          if (!config) return json({ error: '缺少配置' }, 400);
          
          await env.DB.prepare("INSERT INTO integrations (service, config, enabled) VALUES (?, ?, ?) ON CONFLICT(service) DO UPDATE SET config=excluded.config, enabled=excluded.enabled, updated_at=datetime('now')").bind(service, JSON.stringify(config), enabled !== undefined ? enabled : 1).run();
          return json({ ok: true });
        }

        // DELETE /api/admin/integrations/:service - 删除集成配置
        if (method === 'DELETE' && /^\/api\/admin\/integrations\/[^\/]+$/.test(path)) {
          const service = path.split('/').pop();
          await env.DB.prepare("DELETE FROM integrations WHERE service=?").bind(service).run();
          return json({ ok: true });
        }

        // POST /api/admin/integrations/:service/test - 测试集成连接
        if (method === 'POST' && /^\/api\/admin\/integrations\/[^\/]+\/test$/.test(path)) {
          const service = path.split('/')[4];
          const integration = await env.DB.prepare("SELECT * FROM integrations WHERE service=?").bind(service).first();
          if (!integration) return json({ error: '集成不存在' }, 404);
          
          // 这里可以添加实际的测试逻辑
          return json({ ok: true, service, status: '测试功能待实现' });
        }

        // ── 移动端优化 ──
        // POST /api/admin/push/subscribe - 订阅推送通知
        if (method === 'POST' && path === '/api/admin/push/subscribe') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { endpoint, keys, user_agent } = body;
          if (!endpoint || !keys) return json({ error: '缺少endpoint或keys' }, 400);
          
          await env.DB.prepare("INSERT INTO push_subscriptions (endpoint, keys, user_agent) VALUES (?, ?, ?) ON CONFLICT(endpoint) DO UPDATE SET keys=excluded.keys, user_agent=excluded.user_agent").bind(endpoint, JSON.stringify(keys), user_agent || null).run();
          return json({ ok: true });
        }

        // POST /api/admin/push/unsubscribe - 取消订阅推送通知
        if (method === 'POST' && path === '/api/admin/push/unsubscribe') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { endpoint } = body;
          if (!endpoint) return json({ error: '缺少endpoint' }, 400);
          
          await env.DB.prepare("DELETE FROM push_subscriptions WHERE endpoint=?").bind(endpoint).run();
          return json({ ok: true });
        }

        // POST /api/admin/push/send - 发送推送通知（管理员功能）
        if (method === 'POST' && path === '/api/admin/push/send') {
          let body; try { body = await request.json(); } catch { return json({ error: '无效请求' }, 400); }
          const { title, body: message, url } = body;
          if (!title || !message) return json({ error: '缺少标题或内容' }, 400);
          
          // 获取所有订阅
          const { results: subscriptions } = await env.DB.prepare("SELECT * FROM push_subscriptions").all();
          
          // 这里应该实际发送推送通知，但需要web-push库
          // 暂时返回成功，实际实现需要添加web-push依赖
          return json({ ok: true, sent: subscriptions.length, message: '推送功能需要web-push库支持' });
        }

        // GET /api/admin/push/subscribers - 获取订阅者列表
        if (method === 'GET' && path === '/api/admin/push/subscribers') {
          const { results: subscribers } = await env.DB.prepare("SELECT * FROM push_subscriptions ORDER BY created_at DESC").all();
