...
function animateCardIn(card,delay){if(isLowEnd||reducedMotion){card.classList.add('show');card.classList.remove('card-hidden');return}card.classList.remove('card-hidden');var anim=card.animate([{opacity:0,transform:'translateY(10px)'},{opacity:1,transform:'translateY(0)'}],{duration:220,delay:delay,easing:'cubic-bezier(.2,0,0,1)',fill:'forwards'});anim.onfinish=function(){card.classList.add('show');card.style.opacity='';card.style.transform=''}}
