...
function articleCardHtml(id, art) {
  ...
  const catHtml = catObj
    ? '<span class="tag">' + escHtml(catObj.name) + '</span>'
...
<section class="page" id="pageAbout"><div class="container"><div class="about-center"><div class="about-mono"><img src="/avatar.png" alt="Moriefy" width="72" height="72"></div><h1>Moriefy</h1><p class="about-tagline">${escHtml(config.tagline)}</p><div class="about-divider"></div><div class="about-bio">${(Array.isArray(config.bio) ? config.bio : config.bio.split('\n')).filter(Boolean).map(p => '<p>' + escHtml(p) + '</p>').join('')}</div><div class="about-divider"></div><h3 class="about-section-title">技术栈</h3><div class="about-skill-list">${config.skills.map(s => '<span class="about-skill">' + escHtml(s) + '</span>').join('')}</div><div class="about-divider"></div><div class="about-stats"><div class="about-stat"><span class="about-stat-num">${Object.keys(articles).length}</span><span class="about-stat-label">篇文章</span></div><div class="about-stat"><span class="about-stat-num">${config.categories.length}</span><span class="about-stat-label">个分类</span></div><div class="about-stat"><span class="about-stat-num">${allTags.length}</span><span class="about-stat-label">个标签</span></div></div><div class="about-divider"></div><div class="about-links">${config.links.map(l => '<a class="about-link" href="' + escHtml(l.url) + '" target="_blank" rel="noopener">' + escHtml(l.name) + '</a>').join('')}</div><div class="about-divider"></div><div class="secret-drawer" id="secretDrawer"><div class="secret-drawer-header" onclick="this.parentElement.classList.toggle('open')"><svg viewBox="0 0 24 24" width="18" height="18" style="flex-shrink:0"><rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor" stroke-width="2" fill="none"/><path d="M7 11V7a5 5 0 0 1 10 0v4" stroke="currentColor" stroke-width="2" fill="none"/></svg><span class="secret-drawer-title">还有一件事</span><svg class="secret-drawer-arrow" viewBox="0 0 24 24" width="16" height="16"><polyline points="6 9 12 15 18 9" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="secret-drawer-body"><div class="secret-drawer-content"><p>写博客这件事，最初只是想找个地方把脑子里的想法倒出来。没想到倒着倒着，就倒出了三十多篇。</p><p>如果你读到了这里，说明你和我一样，对某些事情有一种不讲道理的好奇心。这种好奇心不需要理由，也不需要目的。它就是存在。</p><p>感谢你的时间。下次见。</p></div></div></div></div></div></section>
